...
import { WalletKit, WalletKitTypes, IWalletKit, isPaymentLink } from '@reown/walletkit';
...
const mmkvStorage = {
  ...
  removeItem: async (key: string) => { _mmkv.remove(key); },
  ...
  getEntries: async <T = unknown>() => {
    ...
    return keys.map((key) => {
      ...
      try { parsed = val ? JSON.parse(val) as T : null; } catch { /* ignore */ }
...
class WalletConnectService {
  ...
  async getRequiredPaymentActions(paymentId: string, optionId: string) {
    return this.walletKit!.pay.getRequiredPaymentActions({ paymentId, optionId });
  }

  async confirmPayment(paymentId: string, optionId: string, signatures: string[]) {
    return this.walletKit!.pay.confirmPayment({ paymentId, optionId, signatures });
  }
}
