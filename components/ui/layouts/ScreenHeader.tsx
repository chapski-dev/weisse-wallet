...
export function ScreenHeader({ backLabel = 'Назад', title, right, onBack }: ScreenHeaderProps) {
  ...
  return (
    ...
    >
      <TouchableOpacity onPress={handleBack} activeOpacity={0.7} style={styles.backBtn}>
        ...
        <Text variant="p2" color={colors.primary}>{backLabel}</Text>
      </TouchableOpacity>

      {title ? (
        <Text variant="h5" color="#fff">{title}</Text>
      ) : (
        <Box />
      )}

      {right ? (
        <Box minWidth={60} alignItems="flex-end">{right}</Box>
      ) : (
        <Box w={60} />
      )}
    </Box>
