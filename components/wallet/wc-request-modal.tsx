...
import { useWalletConnect, WCRequest } from '@/providers/walletconnect-provider';
import { useAppTheme } from '@/theme/theme';
import { WalletKitTypes } from '@reown/walletkit';

// ─── Session Proposal Modal ───────────────────────────────────────────────────

function ProposalModal({ proposal }: { proposal: WalletKitTypes.SessionProposal }) {
  ...
  return (
    <Box flex backgroundColor={colors.background} px={20} py={32}>
      <Box alignItems="center" mb={24}>
        ...
        <Text variant="h3" center mt={12}>{name}</Text>
        <Text variant="p3" colorName="label" center mt={4}>{url}</Text>
        {description ? (
          <Text variant="p3" colorName="label" center mt={8} style={{ lineHeight: 20 }}>
            {description}
          </Text>
        ) : null}
      </Box>
      ...
      <Box
        borderRadius={12} p={16} mb={24}
        backgroundColor={colors.grey_100}
        style={{ borderWidth: 1, borderColor: colors.border }}
      >
        <Text variant="p3-semibold" mb={8}>Разрешения</Text>
        {['Просматривать адрес кошелька', 'Запрашивать подпись транзакций', 'Запрашивать подпись сообщений'].map((p) => (
          <Box key={p} row gap={8} alignItems="center" mb={6}>
            <Ionicons name="checkmark-circle" size={16} color={colors.success} />
            <Text variant="p3" colorName="label">{p}</Text>
          </Box>
        ))}
      </Box>
      ...
      <Box gap={12}>
        ...
        <Button type="outline" onPress={rejectProposal}>Отклонить</Button>
      </Box>
    </Box>
  );
}
  ...
  return (
    <Box flex backgroundColor={colors.background} px={20} py={32}>
      <Box alignItems="center" mb={24}>
        ...
        <Text variant="h3" center>{getMethodLabel(request.method)}</Text>
        <Text variant="p3" colorName="label" center mt={4}>{request.peerName}</Text>
      </Box>
      ...
      >
        <Text variant="p3-semibold" mb={8}>Данные запроса</Text>
        <ScrollView>
          <Text variant="p4" colorName="label" style={{ fontFamily: 'monospace', lineHeight: 18 }}>
            {getParamsPreview()}
          </Text>
        </ScrollView>
      </Box>
      ...
      <Box gap={12}>
        ...
        <Button type="outline" onPress={rejectRequest}>Отклонить</Button>
      </Box>
    </Box>
  );
}
...
export function WCRequestModal() {
  const { pendingProposal, pendingRequest, rejectProposal, rejectRequest } = useWalletConnect();
