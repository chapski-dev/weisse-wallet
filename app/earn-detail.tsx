...
function YieldCard({
	...
	return (
		...
		>
			...
			<Text variant="h4" color="#10B981" numberOfLines={1} adjustsFontSizeToFit>
				{usd}
			</Text>
			<Text
				variant="label"
	...
	return (
		<>
			...
			>
				...
				<Text variant="p4-semibold" color={valueColor}>
					{value}
				</Text>
			</Box>
		...
		<Box flex backgroundColor="#0A0F1E">
			...
			>
				...
				<Box
					...
					borderWidth={1}
					borderColor={protocol.heroBorder}
					style={{
				...
				>
					...
					<Box row justifyContent="space-between" alignItems="center">
						<Box
							...
							alignItems="center"
							justifyContent="center"
							backgroundColor={protocol.iconBg}
						>
							<Ionicons
								name={protocol.iconName}
								size={22}
						...
						<Box
							...
							backgroundColor={protocol.iconBg}
						>
							<Text variant="p4-semibold" color={protocol.iconColor}>
								{protocol.badge}
							</Text>
						</Box>
					</Box>
					...
					<Box alignItems="center" gap={4}>
						<Text variant="h1" color={protocol.iconColor}>
							{protocol.apy}
						</Text>
				...
				>
					<Box
						...
						borderWidth={1}
						borderColor={`${protocol.iconColor}50`}
					>
						<Box
							w={40}
							h={40}
							...
							backgroundColor={protocol.iconBg}
						>
							<Text variant="h5" color={protocol.iconColor}>
								Ξ
							</Text>
						</Box>
				...
				<Box row gap={10} h={100}>
					<YieldCard
						label="ЗА ДЕНЬ"
						usd={fmt(dayUsd)}
						eth={fmtEth(ethPerYear / 365)}
					/>
					<YieldCard
						label="ЗА МЕСЯЦ"
						usd={fmt(monthUsd)}
						eth={fmtEth(ethPerYear / 12)}
					/>
					<YieldCard
						label="ЗА ГОД"
						usd={fmt(yearUsd)}
						eth={fmtEth(ethPerYear)}
						highlighted
					/>
				</Box>
				...
				>
					...
					<DetailRow
						label="Разблокировка"
						value={protocol.unlock}
						valueColor={protocol.unlockColor}
						last
					/>
				</Box>
				...
				<TouchableOpacity onPress={handleStake} activeOpacity={0.85}>
					...
					>
						...
						<Text variant="p1-semibold" color="#FFFFFF">
							Застейкать
						</Text>
					</Box>
				</TouchableOpacity>
