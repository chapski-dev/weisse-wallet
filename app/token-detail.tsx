...
const PriceChart = React.memo(function PriceChart({
	...
	return (
		<View
			...
			style={{ width: CHART_W, height: CHART_H + 30 }}
		>
		...
		<>
			<Box row alignItems="center" gap={12} py={12} onPress={onPress}>
				...
				<Box flex gap={3}>
					...
					<Text variant="caption" color="#7A7A9A">
						{shortenAddr(addr)} · {fmtDate(tx.timestamp)}
					</Text>
				</Box>
				<Box alignItems="flex-end" gap={3}>
					<Text variant="p3-semibold" color={amtColor}>
						{prefix}
						{fmtVal(tx.value)} {symbol}
					</Text>
				</Box>
			</Box>
			{!isLast && (
				<Box h={StyleSheet.hairlineWidth} backgroundColor="#1E1E30" />
			)}
		</>
	);
}
...
export default function TokenDetailScreen() {
	...
	const displayTime = scrubPoint
		? fmtAxisDate(scrubPoint.timestamp, range)
		: null;
		...
		<Box flex backgroundColor="#0D0D1C">
			...
			>
				...
				<Box alignItems="center" gap={4} px={20} pt={12} pb={20}>
					...
					{!scrubPoint && (
						<Box
							...
							py={4}
							borderRadius={20}
							backgroundColor={positive ? "#1A2E1A" : "#2E1A1A"}
						>
							<Ionicons
							...
							>
								{positive ? "+" : ""}
								{priceInfo.change24h.toFixed(2)}% за день
							</Text>
						</Box>
					)}
				</Box>
				...
				<Box
					...
					mt={6}
				>
					<Text variant="label" color="#5A5A7A">
						{dateFrom}
					</Text>
					<Text variant="label" color="#5A5A7A">
						─────────────
				...
				>
					...
					<Box row justifyContent="space-between" alignItems="center">
						...
						<Box row alignItems="center" gap={6}>
							...
							>
								{positive ? "+" : "-"}$
								{Math.abs(chgAmt).toLocaleString("en-US", {
