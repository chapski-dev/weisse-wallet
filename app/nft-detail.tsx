  ...
  return (
    <>
      <Box row justifyContent="space-between" alignItems="center" px={20} h={52}>
        <Text variant="p4" color="#6B7280">{label}</Text>
        {children}
      </Box>
...
export default function NFTDetailScreen() {
  ...
  const shareNFT = async () => {
    try {
      await Share.share({ message: `NFT ${nft.name} из коллекции ${nft.collection}` });
    } catch {}
  };
    ...
    <Box flex backgroundColor={colors.background}>
      <ScreenHeader
        ...
        right={
          <TouchableOpacity onPress={shareNFT} activeOpacity={0.7}>
            <Box w={40} h={40} borderRadius={20} alignItems="center" justifyContent="center" backgroundColor="#1F2937">
      ...
      >
          ...
          <Box mx={20} mb={16} style={{ position: 'relative' }}>
            <Box
              h={280} borderRadius={20} overflow="hidden"
            ...
            >
              <Text variant="p4" color={net.text}>{net.symbol}</Text>
              <Text variant="p4-semibold" color="#fff">{net.name.split(' ')[0]}</Text>
            </Box>
          </Box>

          {/* Name + Collection */}
          <Box px={20} mb={16} gap={8}>
            <Text variant="h3" color="#fff">{nft.name}</Text>
            <Box row alignItems="center" gap={6}>
              <Ionicons name="layers-outline" size={14} color="#6B7280" />
              <Text variant="p4" color="#9CA3AF">{nft.collection}</Text>
          ...
          >
            ...
            <DetailRow label="Контракт">
              <Box row alignItems="center" gap={6} onPress={copyContract}>
                <Text variant="caption" color="#9CA3AF">{nft.contractAddress}</Text>
            ...
            <DetailRow label="Token ID">
              <Text variant="p4" fontWeight="500" color="#fff">{nft.tokenId}</Text>
            </DetailRow>

            {/* Standard */}
            <DetailRow label="Стандарт">
              <Box px={8} py={3} borderRadius={6} backgroundColor="#1F2937">
                <Text variant="p4-semibold" color="#9CA3AF">{nft.standard}</Text>
              </Box>
            </DetailRow>
            ...
            <DetailRow label="Сеть" last>
              <Box row alignItems="center" gap={6}>
                ...
                <Text variant="p4" fontWeight="500" color="#fff">{net.name}</Text>
              </Box>
            </DetailRow>
          </Box>
          ...
          <Box px={20} gap={12}>
            ...
            <Button
              ...
              textColor="primary"
              buttonStyle={{ height: 52, borderRadius: 16, backgroundColor: '#0D1829', borderColor: '#1E3A5F' }}
            >
              Открыть на OpenSea
            </Button>
          </Box>
