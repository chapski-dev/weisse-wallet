...
function PillTabBar({ state, navigation }: BottomTabBarProps) {
  ...
  const activeIndex = visibleRoutes.findIndex((r) => state.routes.indexOf(r) === state.index);
  const tabCount = visibleRoutes.length;
  const tabW = pillWidth > 0 ? pillWidth / tabCount : 0;

  const activeTab = TABS.find((t) => t.name === visibleRoutes[activeIndex]?.name);
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background, paddingBottom: insets.bottom + 8 }]}>
      ...
      >
        ...
        {pillWidth > 0 && (
          <Animated.View
            style={[styles.slidingPill, { width: tabW, backgroundColor: activeColor }, slidingStyle]}
          />
        )}
