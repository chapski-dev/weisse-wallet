...
function getTxIconCfg(tx: Transaction): {
	icon: string;
	iconColor: string;
	bg: string;
} {
...
function SummaryCard({ txs }: { txs: Transaction[] }) {
	...
	const recvStr =
		Object.entries(sumBy(received))
			.map(([s, v]) => `+${formatValue(String(v))} ${s}`)
			.join("\n") || "—";
	const sentStr =
		Object.entries(sumBy(sent))
			.map(([s, v]) => `-${formatValue(String(v))} ${s}`)
			.join("\n") || "—";
...
export default function HistoryScreen() {
	...
	const { wallet, networkMode, getAccountsForCurrentMode, isInitialized } =
		useWallet();
		...
		<Box flex backgroundColor={colors.background}>
			...
			{isLoading ? (
				...
				<Box flex alignItems="center" justifyContent="center" gap={12} px={32}>
					...
					<Text variant="p3" color="#6B7280" center>
						{error}
					</Text>
