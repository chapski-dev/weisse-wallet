...
function NFTCard({ item, onPress }: { item: NFTItem; onPress: () => void }) {
	...
	return (
		...
		>
			...
			{/* Info */}
			<Box p={12} gap={4}>
				<Text variant="p3-semibold" color="#fff" numberOfLines={1}>
					{item.name}
				</Text>
...
function EmptyState({ hasQuery }: { hasQuery: boolean }) {
	return (
		<Box alignItems="center" justifyContent="center" gap={12} mt={60}>
			...
			>
				<Ionicons
					name={hasQuery ? "search-outline" : "image-outline"}
					size={32}
					color="#374151"
				/>
			</Box>
		...
		<Box backgroundColor={colors.background}>
				...
				<Box row gap={8}>
					...
					>
						<Ionicons
							name="search-outline"
							size={18}
							color={showSearch ? "#3B82F6" : "#9CA3AF"}
						/>
					</TouchableOpacity>
					...
					<Box style={{ zIndex: 10 }}>
						<TouchableOpacity
							style={[
								styles.iconBtn,
								(showSort || sortBy !== "default") && styles.iconBtnActive,
							]}
							activeOpacity={0.7}
					...
					return (
						...
						>
							<Box
								...
								justifyContent="center"
								backgroundColor={
									isActive ? "#3B82F6" : net ? net.bg : "#1F2937"
								}
								borderWidth={isActive ? 0 : 1}
								borderColor={
									isActive ? "transparent" : net ? net.border : "#30363D"
								}
							>
								<Text
									variant="p4-semibold"
			...
			<FlatList
				...
				renderItem={({ item }) => (
					<NFTCard
						item={item}
						onPress={() => router.push(`/nft-detail?id=${item.id}`)}
					/>
				)}
				ListEmptyComponent={
