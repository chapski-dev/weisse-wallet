...
const STATUS_CFG = {
  confirmed: { bg: '#052E16', color: '#10B981', label: 'Подтверждено', icon: 'checkmark-circle' as const },
  pending:   { bg: '#1C1405', color: '#F59E0B', label: 'В ожидании',   icon: 'time'             as const },
  failed:    { bg: '#1A0505', color: '#EF4444', label: 'Ошибка',       icon: 'close-circle'     as const },
};
...
function DetailRow({
  ...
  return (
    <>
      <Box row alignItems="center" justifyContent="space-between" px={20} h={52}>
        <Text style={styles.rowLabel}>{label}</Text>
        <Box row alignItems="center" gap={6}>{children}</Box>
      </Box>
      {!noDivider && <Box h={StyleSheet.hairlineWidth} backgroundColor="#1F2937" mx={20} />}
    </>
  );
}
...
function CopyBtn({ value }: { value: string }) {
  return (
    <TouchableOpacity onPress={() => Clipboard.setStringAsync(value)} activeOpacity={0.6}>
...
export default function TransactionDetailScreen() {
  ...
  const heroColor    = params.status === 'failed' ? '#6B7280' : isOutgoing ? '#EF4444' : '#10B981';
  const heroBg       = params.status === 'failed' ? '#111827' : isOutgoing ? '#1A0A0A' : '#052E16';
  const heroBorder   = params.status === 'failed' ? '#37415140' : isOutgoing ? '#EF444440' : '#10B98140';
  const heroGlow     = params.status === 'failed' ? '#37415125' : isOutgoing ? '#EF444425' : '#10B98125';
  ...
  return (
    <Box flex backgroundColor={colors.background}>
      <ScreenHeader
        backLabel="История"
        right={
          <TouchableOpacity style={styles.shareBtn} activeOpacity={0.7} onPress={() => explorerTx && Linking.openURL(explorerTx)}>
      ...
      >
        ...
        <Box alignItems="center" pt={24} pb={16}>
          ...
          {/* Icon circle */}
          <Box
            w={72} h={72} borderRadius={36}
            alignItems="center" justifyContent="center"
            backgroundColor={heroBg}
            style={[styles.iconCircle, { borderColor: heroBorder, shadowColor: heroColor }]}
          >
          ...
          <Text style={[{ color: heroColor }]} variant="h2" mt={20}>
            {amountPrefix}{formatValue(params.value)} {net?.symbol ?? ''}
          </Text>

          {/* Status badge */}
          <Box
            row alignItems="center" gap={6}
            px={14} py={6} borderRadius={20}
            backgroundColor={statusCfg.bg}
            mt={12}
          >
            <Ionicons name={statusCfg.icon} size={14} color={statusCfg.color} />
            <Text variant="p4-semibold" color={statusCfg.color}>{statusCfg.label}</Text>
          </Box>
        ...
        >
          ...
          <DetailRow label="Сеть">
            <Text variant="p3" color={net ? '#4B8EF5' : '#9CA3AF'}>{net?.icon ?? ''}</Text>
            <Text variant="p3" color="#fff" fontWeight="500">{net?.name ?? params.network}</Text>
          </DetailRow>

          {/* Токен */}
          <DetailRow label="Токен">
            <Text variant="p3" color="#fff" fontWeight="500">{net?.symbol ?? ''}</Text>
          </DetailRow>
          ...
          <DetailRow label="Комиссия">
            <Text variant="p3" color="#fff" fontWeight="500">
              {params.fee ? `${formatValue(params.fee, 6)} ${net?.symbol ?? ''}` : '—'}
            </Text>
          </DetailRow>
        ...
        {explorerTx && (
          ...
          >
            <Box
              ...
              mx={20} mt={12} h={52} borderRadius={14}
