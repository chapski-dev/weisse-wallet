...
export default function WCConnectScreen() {
  ...
  const processUri = async (raw: string) => {
    ...
    if (!trimmed.startsWith('wc:')) {
      setError('Неверный формат. URI должен начинаться с "wc:" или быть ссылкой WalletConnect Pay');
      return;
    }
  ...
  const renderScanner = () => {
    if (!permission) {
      return (
        <Box flex alignItems="center" justifyContent="center">
          <Text variant="p2" colorName="label">Загрузка...</Text>
        </Box>
      );
    }
    ...
    if (!permission.granted) {
      return (
        <Box flex alignItems="center" justifyContent="center" gap={16} px={32}>
          ...
          <Text variant="h4" center>Нужен доступ к камере</Text>
          <Text variant="p3" center colorName="label">
            Для сканирования QR-кода требуется разрешение на использование камеры
          </Text>
    ...
    return (
      <Box flex>
        ...
        {loading && (
          <Box style={styles.loadingOverlay} alignItems="center" justifyContent="center">
            <Box px={24} py={16} borderRadius={12} backgroundColor={colors.card}>
              <Text variant="p2" colorName="label">Подключение...</Text>
            </Box>
          </Box>
        )}

        {error ? (
          <Box style={styles.errorBanner} px={20} py={12} backgroundColor={colors.error}>
            <Text variant="p3" color="#fff" center>{error}</Text>
            <TouchableOpacity onPress={() => { setError(''); scanned.current = false; }}>
              <Text variant="p3-semibold" color="#fff" center mt={4}>Попробовать снова</Text>
            </TouchableOpacity>
          </Box>
        ) : null}
      </Box>
  ...
  const renderPaste = () => (
    <Box flex px={20} pt={8}>
      <Box
        borderRadius={12}
        style={{ borderWidth: 1, borderColor: error ? colors.error : colors.border }}
        mb={error ? 8 : 16}
      ...
      >
        <Box row alignItems="center" px={16} py={4}>
          <TextInput
            ...
            value={uri}
            onChangeText={(t) => { setUri(t); setError(''); }}
            multiline
            numberOfLines={3}
            autoCapitalize="none"
            autoCorrect={false}
      ...
      {error ? (
        <Text variant="p3" color={colors.error} mb={16}>{error}</Text>
      ) : null}
  ...
  return (
    <Box flex backgroundColor={colors.background}>
      ...
      >
        ...
        <TouchableOpacity
          onPress={() => { setMode(mode === 'scanner' ? 'paste' : 'scanner'); setError(''); scanned.current = false; }}
          style={[styles.iconBtn, { backgroundColor: colors.grey_200 }]}
        >
          <Ionicons
        ...
        <Box px={20} pb={insets.bottom + 16} pt={12} alignItems="center">
          <TouchableOpacity onPress={() => setMode('paste')}>
            <Text variant="p3" colorName="label">
              Нет QR-кода? <Text variant="p3-semibold" color={colors.primary}>Вставить URI</Text>
            </Text>
          </TouchableOpacity>
        </Box>
      )}
