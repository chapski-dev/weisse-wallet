...
function OptionCard({
  ...
  return (
    <TouchableOpacity style={styles.card} onPress={onPress} activeOpacity={0.8}>
      <Box w={52} h={52} borderRadius={26} alignItems="center" justifyContent="center" style={{ backgroundColor: iconBg }}>
        <Ionicons name={icon as any} size={24} color="#3B82F6" />
      </Box>
      <Box flex justifyContent="center" gap={4}>
        <Text variant="p2-semibold" color="#fff">{title}</Text>
        <Text variant="p4" colorName="label">{subtitle}</Text>
      </Box>
...
export default function AddWalletScreen() {
  ...
  return (
    <Box flex backgroundColor="#0A0F1E" style={{ paddingTop: insets.top }}>
      {/* Header */}
      <Box row justifyContent="space-between" alignItems="center" px={20} h={56}>
        <TouchableOpacity style={styles.backBtn} onPress={() => router.back()}>
          <Ionicons name="chevron-back" size={20} color="#3B82F6" />
          <Text variant="p2-semibold" color="#3B82F6">Кошельки</Text>
        </TouchableOpacity>
        <Text variant="p1-semibold" color="#fff">Новый кошелёк</Text>
        <Box w={72} />
      </Box>
