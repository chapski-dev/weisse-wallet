...
type Step = 'loading' | 'confirm' | 'webview' | 'confirming' | 'success' | 'error';
...
export default function WCPayScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const data = await walletConnectService.getPaymentOptions(paymentLink, accounts) as PaymentOptionsResponse;
        ...
        setErrorMessage(e instanceof Error ? e.message : 'Ошибка загрузки вариантов оплаты');
        setStep('error');
      }
    })();
  }, [paymentLink]);
  ...
  const executePayment = useCallback(async () => {
    ...
    try {
      ...
      setErrorMessage(e instanceof Error ? e.message : 'Ошибка при подтверждении оплаты');
      setStep('error');
    }
  }, [paymentData, selectedOption]);
  ...
  const renderContent = () => {
    switch (step) {
      case 'loading':
        return (
          <Box flex alignItems="center" justifyContent="center" gap={16}>
            ...
            <Text variant="p2" colorName="label">Загружаем варианты оплаты...</Text>
          </Box>
        );
        ...
        return (
          <ScrollView showsVerticalScrollIndicator={false}>
            ...
            <Box alignItems="center" mb={24}>
              {merchant?.iconUrl ? (
                <Image source={{ uri: merchant.iconUrl }} style={styles.merchantIcon} />
              ) : (
                <Box
                  w={64} h={64} borderRadius={32}
                  alignItems="center" justifyContent="center"
                  backgroundColor={colors.grey_200} mb={12}
                >
                  <Ionicons name="storefront-outline" size={32} color={colors.label} />
                </Box>
              )}
              <Text variant="h3" center mt={8}>{merchant?.name ?? 'Оплата'}</Text>
            </Box>

            {/* Варианты оплаты */}
            <Text variant="p3-semibold" mb={8}>Выберите вариант оплаты</Text>
              ...
              return (
                <TouchableOpacity
                  ...
                  style={[
                    styles.optionRow,
                    {
                      backgroundColor: isSelected ? colors.primary_700_15 : colors.card,
                      borderColor: isSelected ? colors.primary : colors.border,
                    },
                  ]}
                >
                  {option.amount.display.iconUrl ? (
                    <Image source={{ uri: option.amount.display.iconUrl }} style={styles.tokenIcon} />
                  ) : (
                    <Box w={36} h={36} borderRadius={18} backgroundColor={colors.grey_200} alignItems="center" justifyContent="center">
                      <Text variant="p3">{option.amount.display.assetSymbol[0]}</Text>
                    </Box>
                  )}
                  <Box flex ml={12} gap={2}>
                    <Text variant="p2-semibold">{formatAmount(option)}</Text>
                    <Text variant="p4" colorName="label">
                      {option.amount.display.networkName ?? option.amount.display.assetName}
                      {option.etaS ? ` · ~${Math.ceil(option.etaS)}с` : ''}
                    </Text>
                  </Box>
                  {option.collectData && (
                    <Box px={6} py={2} borderRadius={6} backgroundColor={colors.warning_500}>
                      <Text variant="p4-semibold" color="#fff">Данные</Text>
                    </Box>
                  )}
                  {isSelected && (
            ...
            <Box mt={24} gap={12}>
              ...
              <Button type="outline" onPress={() => router.back()}>Отмена</Button>
            </Box>
          </ScrollView>
        );
      ...
      case 'webview':
        return (
          <Box flex>
            <Text variant="p3" colorName="label" mb={8}>Заполните необходимые данные</Text>
            <WebView
            ...
            />
            <Button type="outline" onPress={() => router.back()} wrapperStyle={{ marginTop: 12 }}>
              Отмена
            </Button>
          </Box>
        );
      ...
      case 'confirming':
        return (
          <Box flex alignItems="center" justifyContent="center" gap={16}>
            <ActivityIndicator size="large" color={colors.primary} />
            <Text variant="p2" colorName="label">Подтверждаем оплату...</Text>
          </Box>
        );
      ...
      case 'success':
        return (
          <Box flex alignItems="center" justifyContent="center" gap={16}>
            ...
            <Text variant="h3" center>Оплата прошла!</Text>
            <Text variant="p2" center colorName="label">Транзакция успешно подтверждена</Text>
            <Button onPress={() => router.back()} wrapperStyle={{ marginTop: 8 }}>
              Готово
            </Button>
          </Box>
        );
      ...
      case 'error':
        return (
          <Box flex alignItems="center" justifyContent="center" gap={16}>
            ...
            <Text variant="h3" center>Ошибка оплаты</Text>
            <Text variant="p2" center colorName="label">{errorMessage}</Text>
            <Box row gap={12} mt={8}>
              <Box flex>
                <Button type="outline" onPress={() => router.back()}>Закрыть</Button>
              </Box>
              <Box flex>
                <Button onPress={() => { setStep('loading'); setErrorMessage(''); }}>
                  Повторить
                </Button>
              </Box>
            </Box>
  ...
  return (
    <Box flex backgroundColor={colors.background} px={20} pt={insets.top + 16} pb={insets.bottom + 24}>
      {/* Header */}
