...
export default function SendNFTScreen() {
  ...
  if (!nft) {
    return (
      <Box flex alignItems="center" justifyContent="center" backgroundColor={colors.background}>
        <Text variant="p2" colorName="label">NFT не найден</Text>
      </Box>
    );
  }
  ...
  const isRealNFT = !!nft.networkId && !!nft.tokenIdRaw && !!nft.contractAddress
  ...
  const addressValid = isRealNFT ? isValidEVMAddress(trimmedAddress) : trimmedAddress.length > 0;
  ...
  const executeSend = async () => {
    ...
    try {
      ...
      Alert.alert(
        ...
        [
          explorerUrl
            ? { text: 'Открыть в Explorer', onPress: () => Linking.openURL(explorerUrl) }
            : null,
          { text: 'OK', onPress: () => router.back() },
        ].filter(Boolean) as any,
      );
    } catch (err: any) {
      const message: string = err?.shortMessage ?? err?.message ?? 'Неизвестная ошибка';
  ...
  return (
    <Box flex backgroundColor={colors.background}>
      ...
      <Box
        ...
        mx={20} mt={4} mb={24}
        p={12} borderRadius={16}
        backgroundColor="#111827"
        borderWidth={1} borderColor="#1F2937"
      >
        <Box
          w={52} h={52} borderRadius={12}
        ...
        <Box flex gap={4}>
          <Text variant="p3-semibold" color="#F9FAFB">{nft.name}</Text>
          <Text variant="caption" color="#6B7280">@ {nft.collection}</Text>
          {fromAccount && (
            <Text variant="caption" color="#4B5563" numberOfLines={1}>
              С: {fromAccount.address.slice(0, 8)}...{fromAccount.address.slice(-6)}
            </Text>
          )}
        </Box>
        ...
        <Box
          ...
          px={10} py={5} borderRadius={10}
          backgroundColor={net.bg}
          borderWidth={1} borderColor={net.border}
        >
          <Text fontSize={13} color={net.text}>{net.symbol}</Text>
          <Text variant="caption-medium" color="#fff">{net.name.split(' ')[0]}</Text>
        </Box>
      </Box>
      ...
      <Box
        mx={20} borderRadius={16}
        backgroundColor="#111827"
        borderWidth={1}
        borderColor={trimmedAddress.length > 0 && !addressValid ? '#EF444450' : '#1F2937'}
        overflow="hidden"
      >
        <Box row alignItems="center" gap={8} px={14} h={52}>
          <TextInput
            ...
            autoCapitalize="none"
            autoCorrect={false}
            style={{ flex: 1, color: '#F9FAFB', fontSize: 15, fontFamily: 'monospace' }}
          />

          <TouchableOpacity onPress={handlePaste} activeOpacity={0.8}>
            <Box px={12} py={6} borderRadius={8} backgroundColor="#0D2E1F" borderWidth={1} borderColor="#1A4030">
              <Text variant="p4-semibold" color="#10B981">Вставить</Text>
            </Box>
          </TouchableOpacity>
        </Box>
        ...
        {trimmedAddress.length > 0 && !addressValid && (
          <Box px={14} pb={10}>
            <Text variant="caption" color="#EF4444">Неверный формат адреса</Text>
          </Box>
        )}

        <Box h={1} backgroundColor="#1F2937" />

        <Box row justifyContent="space-between" alignItems="center" px={14} h={44}>
          <Text variant="caption" color="#6B7280">Комиссия сети:</Text>
          <Text variant="caption-medium" color="#9CA3AF">{feeLabel}</Text>
        </Box>
      </Box>
      ...
      <Box
        row gap={10}
        mx={20} mt={16} p={14} borderRadius={14}
        backgroundColor="#1C1200"
        borderWidth={1} borderColor="#F59E0B40"
      >
        <Ionicons name="warning-outline" size={18} color="#F59E0B" style={{ marginTop: 1 }} />
        <Box flex gap={4}>
          <Text variant="caption-medium" color="#F59E0B">NFT будет отправлен безвозвратно.</Text>
          <Text variant="caption" color="#D97706">Внимательно проверьте адрес получателя.</Text>
        </Box>
      </Box>

      <Box px={20} pb={insets.bottom + 16} pt={16} style={{ marginTop: 'auto' }}>
        <Button
          onPress={handleSend}
